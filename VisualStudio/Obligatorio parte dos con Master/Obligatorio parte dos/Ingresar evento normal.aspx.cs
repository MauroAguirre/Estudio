...
{
    public partial class Ingresar_evento_normal : System.Web.UI.Page
    {
        ...
        protected void Agregar_Click(object sender, EventArgs e)
        {
            ...
            check[1] = false;
            check[2] = false;
            if (txtCliente.Text == "" || txtDescripcion.Text == "" || (int.Parse(numAsistentes.Text)) < 1 || (int.Parse(numAsistentes.Text)) > 10 || (int.Parse(numDuracion.Text)) < 1 || dteFecha.Text == "")
                check[0] = false;
            if (check[0] == true)
            {
            ...
            if (check[1] && check[2])
            {
                ...
                Normal nuevoEvento = new Normal(turno, int.Parse(numDuracion.Text), DateTime.Parse(dteFecha.Text), txtDescripcion.Text, txtCliente.Text, int.Parse(numAsistentes.Text), int.Parse(numPersonas.Text), organizador, newServiciosComprados);
                Eventos2017 eventos2017 = Session["Sistema"] as Eventos2017;
                eventos2017.AgregarEvento(nuevoEvento);
                Session["Sistema"] = eventos2017;
                lblRespuesta.Text = "Evento ingresado correctamente";
            }
            else
        ...
        protected void btnAgregarBebidas_Click(object sender, EventArgs e)
        {
            ...
            {
                if (int.Parse(numBebidas.Text) > 0 && int.Parse(numBebidas.Text) <= int.Parse(numPersonas.Text) && int.Parse(numBebidas.Text) < 11)
                    correcto = true;
            }
            if (correcto)
            {
        ...
        protected void btnAgregarComidas_Click(object sender, EventArgs e)
        {
            ...
            if (esNumero && numPersonas.Text != "")
            {
                if (int.Parse(numComidas.Text) > 0 && int.Parse(numComidas.Text) <= int.Parse(numPersonas.Text) && int.Parse(numComidas.Text) < 11)
                    correcto = true;
            }
            if (correcto)
            {
        ...
        protected void btnAgregarSonido_Click(object sender, EventArgs e)
        {
            ...
            if (esNumero && numPersonas.Text != "")
            {
                if (int.Parse(numSonido.Text) > 0 && int.Parse(numSonido.Text) <= int.Parse(numPersonas.Text) && int.Parse(numSonido.Text) < 11)
                    correcto = true;
            }
            if (correcto)
            {
