            ...
            if (ubicacion != -1)
            {
                foreach (Evento evento in organizadores[ubicacion].DarEventos())
                {
                    ...
                    if(evento.GetType() == typeof(Normal))
                        Console.WriteLine("El costo total es de: "+evento.CostoTotal()+300);//el 300 es el costo de limpieza
                    else
                        Console.WriteLine("El costo total es de: " + (evento.CostoTotal() + ((evento.CostoTotal() / 100) * evento.DarAumento())));//como saco el aumento? proximamente en cines
                }
                Console.WriteLine("Ingrese enter para continuar");
                Console.ReadKey();
            }
